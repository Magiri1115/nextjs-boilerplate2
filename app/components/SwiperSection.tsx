...
export default function SwiperSection({ items }: { items: Item[] }) {
        ...
        {items.map((item, i) => (
          <SwiperSlide key={i}>
            <div className="flex flex-col items-center justify-center h-full text-white text-center px-6 bg-transparent">
              <img src={item.icon} alt={item.title} className="w-20 h-20 mb-6" />
              <h2 className="text-3xl font-bold mb-4">{item.title}</h2>
              <p className="text-lg max-w-xs">{item.description}</p>
            </div>
          </SwiperSlide>
        ))}
      </Swiper>
